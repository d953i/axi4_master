...
(
    ...
    parameter integer P_AWUSER_WIDTH = 0,                                       //Width of User Write Address Bus
    parameter integer P_ARUSER_WIDTH = 0,                                       //Width of User Read Address Bus
    parameter integer P_WUSER_WIDTH  = 0,                                       //Width of User Write Data Bus
    parameter integer P_RUSER_WIDTH  = 0,                                       //Width of User Read Data Bus
    parameter integer P_BUSER_WIDTH  = 0                                        //Width of User Response Bus
)
(
    ...
    output wire [P_ID_WIDTH - 1:0] M_AXI_AWID,                                  //Master Interface Write Address ID
    output wire [P_ADDR_WIDTH - 1:0] M_AXI_AWADDR,                              //Master Interface Write Address
    ...
    output wire [P_DATA_WIDTH-1 : 0] M_AXI_WDATA,                               //Master Interface Write Data.
    output wire [P_DATA_WIDTH/8-1 : 0] M_AXI_WSTRB,                             //Write strobes. This signal indicates which byte lanes hold valid data. There is one write strobe bit for each eight bits of the write data bus.
    ...
    output wire [P_ID_WIDTH-1 : 0] M_AXI_ARID,                                  //Master Interface Read Address.
    output wire [P_ADDR_WIDTH-1 : 0] M_AXI_ARADDR,                              //Read address. This signal indicates the initial address of a read burst transaction.
    ...
    input  wire [P_ID_WIDTH-1 : 0] M_AXI_RID,                                   //Read ID tag. This signal is the identification tag for the read data group of signals generated by the slave.
    input  wire [P_DATA_WIDTH-1 : 0] M_AXI_RDATA,                               //Master Read Data
    ...
    input  wire [P_ID_WIDTH-1 : 0] M_AXI_BID,                                   //Master Interface Write Response.

    output wire [P_AWUSER_WIDTH - 1:0] M_AXI_AWUSER,                            //Optional User-defined signal in the write address channel.
    output wire [P_WUSER_WIDTH - 1: 0] M_AXI_WUSER,                             //Optional User-defined signal in the write data channel.
    output wire [P_ARUSER_WIDTH - 1: 0] M_AXI_ARUSER,                           //Optional User-defined signal in the read address channel.
    input  wire [P_RUSER_WIDTH - 1: 0] M_AXI_RUSER,                             //Optional User-defined signal in the read data channel.
    input  wire [P_BUSER_WIDTH - 1: 0] M_AXI_BUSER                              //Optional User-defined signal in the write response channel
);
...
reg [P_ADDR_WIDTH - 1:0] axi_awaddr;
reg axi_awvalid;
reg [P_DATA_WIDTH - 1:0] axi_wdata;
reg axi_wlast;
reg axi_wvalid;
reg axi_bready;
reg [P_ADDR_WIDTH - 1:0] axi_araddr;
reg axi_arvalid;
reg axi_rready;
...
assign M_AXI_AWLEN= P_BURST_LEN - 1;                                            //Size should be P_DATA_WIDTH, in 2^SIZE bytes, otherwise narrow bursts are used
assign M_AXI_AWSIZE = clogb2((P_DATA_WIDTH / 8) - 1);                          //INCR burst type is usually used, except for keyhole bursts
...
assign M_AXI_WSTRB = {(P_DATA_WIDTH / 8){1'b1}};
...
assign M_AXI_ARLEN = P_BURST_LEN - 1;                                           //Size should be P_DATA_WIDTH, in 2^n bytes, otherwise narrow bursts are used
assign M_AXI_ARSIZE = clogb2((P_DATA_WIDTH / 8) - 1);                           //INCR burst type is usually used, except for keyhole bursts
...
always @(posedge M_AXI_ACLK) begin
    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_write == 1'b1) begin
        write_index <= 0;
    end else if (wnext && (write_index != P_BURST_LEN-1)) begin
...
always @(posedge M_AXI_ACLK) begin
    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_read) begin
        read_index <= 0;
    end else if (rnext && (read_index != P_BURST_LEN-1)) begin
...
always @(posedge M_AXI_ACLK) begin
    ...
    //else if (M_AXI_BVALID && axi_bready && (write_burst_counter == {(C_NO_BURSTS_REQ-1){1}}) && axi_wlast)
    else if (M_AXI_RVALID && axi_rready && (read_index == P_BURST_LEN-1) && (read_burst_counter[C_NO_BURSTS_REQ]))
        reads_done <= 1'b1;
    else
